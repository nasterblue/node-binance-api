...
let api = function Binance( options = {} ) {
    ...
    const fUserDataStreamExpiredConvertData = data => {
        let {
            e: eventType,
            E: eventTime,
            T: transaction, // transaction time
   ...
   */
    ...
    const userFutureDataHandler = data => {
        ...
        if ( type === 'MARGIN_CALL' ) {
            ...
            if ( Binance.options.future_listenKeyExpired_callback ) {
                Binance.options.future_listenKeyExpired_callback(fUserDataStreamExpiredConvertData(data));
            }
        } else {
   ...
   */
     ...
     */
        ...
        websockets: {
             ...
             * @param {Function} subscribed_callback - subscription callback
             */
            userFutureData: function userFutureData( margin_call_callback, account_update_callback = undefined, order_update_callback = undefined, subscribed_callback = undefined, account_config_update_callback = undefined, future_listenKeyExpired_callback = undefined ) {
                const url = ( Binance.options.test ) ? fapiTest : fapi;

                let reconnect = () => {
                    if ( Binance.options.reconnect ) userFutureData( margin_call_callback, account_update_callback, order_update_callback, subscribed_callback, account_config_update_callback, future_listenKeyExpired_callback )
                }
                ...
                apiRequest( url + 'v1/listenKey', {}, function ( error, response ) {
                    ...
                    Binance.options.future_listenKeyExpired_callback = future_listenKeyExpired_callback;
